...
function drawTree(drawingName, selectString, treeData) {
	...
	var smth;
	var rawFile = new XMLHttpRequest();
    rawFile.open("GET", "treeData.json", false);
    rawFile.onreadystatechange = function ()
    {
        if(rawFile.readyState === 4)
        {
	...
	var x2js = new X2JS();
	var xmlText = allText;
	var jsonFile = x2js.xml_str2json( xmlText );*/
	...
	var margin = {
		...
		.append("g")
		.attr("class", "drawarea".concat(drawingName))
		.append("g")
		.attr("transform", "translate(" + margin.left + "," + margin.top + ")");

		//d3.json(dataURL, function(error, treeData) {
			root = treeData[0];
			root.x0 = height / 2;
			root.y0 = 0;
			update(root);
		...
		//d3.select(selectString)
		var str1 = ".";
		d3.select(".drawarea".concat(drawingName))
